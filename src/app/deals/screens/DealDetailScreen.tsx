    ...
    <View className="flex-1 bg-gray-50">
      ...
      <View className="bg-green-800 pt-12 pb-6 px-4 overflow-hidden">
        ...
        <Text
          ...
          style={{ color: '#E8A838', letterSpacing: 1 }}
        >
          {deal.id}
        </Text>
        ...
        <Text className="text-orange-400 text-base font-bold mt-1">
          {deal.amount}
        </Text>
      </View>
      ...
      >
        ...
        >
          <View className="flex-row gap-3.5 mb-3.5">
            <View
              className="bg-green-50 rounded-xl items-center justify-center"
            ...
            <View className="flex-1">
              {[
                { label: t('deal.quantity'), val: deal.qty },
                { label: t('deal.rate'), val: deal.rate },
                { label: t('deal.location'), val: deal.location },
              ].map(r => (
        ...
        >
          <View className="flex-row justify-between mb-3">
            ...
            <Text className="text-xs font-bold" style={{ color }}>
              {t('deals.stage', { stage: deal.stage })}
            </Text>
          </View>
          ...
          <Text className="text-xs font-bold mb-3.5" style={{ color }}>
        ...
        >
          <Text className="text-gray-900 text-sm font-extrabold mb-1">
            {t('deal.paymentSchedule')}
          </Text>
          {deal.payments.map((p: any) => (
            <View
              key={p.label}
              className="flex-row justify-between items-start py-2.5 border-b border-gray-100"
            >
              ...
              <View className="items-end">
                ...
                >
                  <Text
                    className={`text-xs font-bold ${
                      p.status === 'Paid' ? 'text-green-700' : 'text-amber-600'
                    }`}
                  >
                    {p.status === 'Paid' ? t('deal.paid') : t('deal.pending')}
                  </Text>
                </View>
              </View>
            </View>
        ...
        >
          <Text className="text-gray-900 text-sm font-extrabold mb-3">
            {t('deal.actions')}
          </Text>
