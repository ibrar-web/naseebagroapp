...
const PaymentMethodsScreen = ({ navigation }: any) => {
  ...
  return (
    <View className="flex-1 bg-gray-50">
      ...
      >
        {accounts.map(acc => (
          <View key={acc.id} className="bg-white rounded-2xl overflow-hidden mb-4" style={CARD_SHADOW}>
            {/* Bank header strip */}
            <View
              className="flex-row items-center justify-between px-4 py-3"
              style={{ backgroundColor: (BANK_COLORS[acc.bank] ?? '#1A6B34') + '18' }}
            >
            ...
            <View className="px-4 py-3 border-t border-gray-100">
              <Text className="text-xs text-gray-400 mb-0.5 font-semibold uppercase tracking-wider">IBAN</Text>
        ...
        <TouchableOpacity
          className="bg-green-700 rounded-2xl py-4 items-center flex-row justify-center gap-2 mt-1"
