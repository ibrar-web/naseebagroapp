    ...
    return (
      <View className="flex-1 bg-gray-50 items-center justify-center px-6 gap-4">
        ...
        <Text className="text-gray-900 text-2xl font-extrabold text-center">
          {isBuyer ? t('post.demandPosted') : t('post.listingCreated')}
        </Text>
        <Text className="text-gray-500 text-sm text-center leading-5">
          {isBuyer ? t('post.sellerNotify') : t('post.listingReview')}
        </Text>
        <TouchableOpacity
    ...
    >
      ...
      <View className="bg-green-800 pt-12 pb-6 px-4 overflow-hidden">
        ...
        <Text className="text-white text-2xl font-extrabold">
          {isBuyer ? t('post.postDemandTitle') : t('post.createListingTitle')}
        </Text>
        <Text className="text-green-300 text-sm mt-1">
          {isBuyer ? t('post.postDemandSub') : t('post.createListingSub')}
        </Text>
      </View>
      ...
      >
              ...
              >
                <Text
                  className={`text-sm font-semibold ${
                    commodity === c ? 'text-white' : 'text-gray-600'
                  }`}
                >
                  {c}
                </Text>
        ...
        >
          <Text className="text-gray-900 text-sm font-extrabold px-4 pt-4 pb-2">
            {isBuyer ? t('post.budget') : t('post.askingPrice')}
          </Text>
        ...
        >
          ...
          <TextInput
            ...
            placeholder={
              isBuyer
                ? t('post.buyerNotesPlaceholder')
                : t('post.sellerNotesPlaceholder')
            }
            placeholderTextColor="#9CA3AF"
            value={notes}
        ...
        >
          <Text className="text-white text-base font-bold">
            {isBuyer ? t('post.postDemand') : t('post.submitListing')}
          </Text>
        </TouchableOpacity>
      </ScrollView>
    </KeyboardAvoidingView>
