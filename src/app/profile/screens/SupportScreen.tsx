...
const CONTACT = [
  ...
  { icon: '📞', label: 'Helpline',  sub: '0800-12345 (Free)', color: '#3B82F6' },
];
  ...
  return (
    <View className="flex-1 bg-gray-50">
      ...
      <ScrollView className="flex-1" contentContainerStyle={{ padding: 16, gap: 14, paddingBottom: 40 }}
        ...
        {/* Contact channels */}
        <Text className="text-xs font-bold text-gray-400 uppercase tracking-wider px-1">Contact Us</Text>
        ...
        {/* FAQs */}
        <Text className="text-xs font-bold text-gray-400 uppercase tracking-wider px-1">FAQs</Text>
        <View className="bg-white rounded-2xl overflow-hidden"
              style={{ shadowColor: '#000', shadowOpacity: 0.06, shadowRadius: 8, elevation: 3 }}>
          {FAQS.map((faq, idx) => (
            <View key={idx}
                  className={idx < FAQS.length - 1 ? 'border-b border-gray-100' : ''}>
              <TouchableOpacity
              ...
              >
                ...
                <Text className="text-gray-300 text-base">{open === idx ? '∨' : '›'}</Text>
              </TouchableOpacity>
              {open === idx && (
