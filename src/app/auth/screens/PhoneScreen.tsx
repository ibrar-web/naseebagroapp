  ...
  return (
    ...
    >
      ...
      <View className="bg-green-800 pt-12 pb-8 px-5 overflow-hidden">
        ...
        <Text className="text-gold text-xs font-bold tracking-widest mb-2">STEP 1 OF 2</Text>
        <Text className="text-white text-3xl font-extrabold leading-9">Enter Your{'\n'}Phone Number</Text>
        <Text className="text-green-300 text-sm mt-2">We'll send you a verification code</Text>
      </View>
      ...
      <ScrollView contentContainerStyle={{ padding: 16, paddingTop: 24 }}
        ...
        <View className="bg-white rounded-2xl p-4 mb-5"
          ...
          <Text className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">Mobile Number</Text>
          ...
          <Text className="text-gray-400 text-xs mt-2">Enter your 10-11 digit mobile number</Text>
        </View>

        <TouchableOpacity
          onPress={() => navigation.navigate('OTP', { phone })}
          className={`py-4 rounded-2xl items-center ${canContinue ? 'bg-green-700' : 'bg-green-700 opacity-40'}`}
          activeOpacity={0.88}
          disabled={!canContinue}
          style={{ shadowColor: '#1A6B34', shadowOpacity: canContinue ? 0.3 : 0, shadowRadius: 8, elevation: canContinue ? 4 : 0 }}
        >
