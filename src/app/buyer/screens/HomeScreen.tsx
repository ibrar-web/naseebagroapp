...
const HomeScreen = ({ navigation }: any) => {
  ...
  return (
    <View className="flex-1 bg-gray-50">
      ...
      <View className="bg-green-800 px-4 pt-12 pb-4 overflow-hidden">
        ...
        <View className="flex-row justify-between items-center">
          <View className="flex-row items-center gap-3">
            <View
              className="w-11 h-11 rounded-2xl bg-orange-500 items-center justify-center"
      ...
      {/* ── Stats strip ────────────────────────────────────────── */}
      <View
        className="flex-row bg-white px-4 py-2.5 gap-2 border-b border-gray-100"
        style={{
          shadowColor: '#000',
          shadowOpacity: 0.05,
          shadowRadius: 4,
      ...
      >
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <View
              className="bg-white rounded-2xl p-3.5 w-32"
              style={{
                shadowColor: '#000',
                shadowOpacity: 0.05,
                shadowRadius: 6,
            ...
            >
              ...
              <Text className="text-gray-900 text-sm font-extrabold mt-1">
                {item.price}
              </Text>
              <Text
        ...
        {isBuyer ? (
          <>
            ...
            <View className="flex-row flex-wrap gap-2.5 px-4">
              {CATEGORIES.map(cat => (
                <View
                  key={cat.name}
                  className="bg-white rounded-2xl items-center py-4 gap-2"
                  style={{
                    width: (W - 48) / 2,
                    shadowColor: '#000',
                    shadowOpacity: 0.04,
          ...
          <>
            {/* Earnings */}
            <View
              className="mx-4 mt-5 bg-green-800 rounded-2xl p-4"
              style={{
                shadowColor: '#145228',
                shadowOpacity: 0.2,
                shadowRadius: 8,
            ...
            >
              ...
              <View className="flex-row gap-5 mt-3">
                {[
                  { l: t('home.released'), v: '₨640K', c: 'text-white' },
                  { l: t('home.pending'), v: '₨250K', c: 'text-orange-400' },
                  { l: t('home.thisWeek'), v: '₨120K', c: 'text-white' },
                ].map(s => (
                  <View key={s.l}>
            ...
            {/* Quick actions */}
            <Text className="text-gray-900 text-base font-extrabold px-4 pt-5 pb-2.5">
              {t('home.quickActions')}
            </Text>
            <View className="flex-row flex-wrap gap-2.5 px-4">
              {QUICK_ACTIONS.map(a => (
                <TouchableOpacity
                  key={a.labelKey}
                  className="bg-white rounded-2xl p-3.5 gap-2"
                  style={{
                    width: (W - 48) / 2,
                    shadowColor: '#000',
                    shadowOpacity: 0.05,
                ...
                >
                  ...
                  <Text className="text-gray-400 text-xs">{t(a.subKey)}</Text>
                </TouchableOpacity>
              ))}
            </View>
          </>
